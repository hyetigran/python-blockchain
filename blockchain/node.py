...
def get_balance():
    ...
    if balance != None:
        response = {
            'message': 'Fetched balance success',
            'funds': balance
        }
        return jsonify(response), 201
    else:
...
def resolve_conflicts():
    ...
    if replaced:
        ...
        response = {'message': "local chian kept"}
...
def broadcast_block():
    ...
    if block['index'] == blockchain.chain[-1].index + 1:
        ...
        response = {'message': 'Blockchain seems to differ, block not added'}
